...
public class LogisticReg {
    ...
    public static void main(String[] args) {
        ...
        for (int j = 0; j < numIters; j++) {
            ...
            Matrix yTrain = getActualY(trainPoints, randLine);
//            Utilities.writeMatrixToFile(xVals, yActual, "test.csv");
            Matrix fitW = sgd(trainPoints, yTrain);
//            Utilities.writeMatrixToFile(fitW, "testW.csv");
            Matrix testPoints = Utilities.genPointsWithIntercept(1000, 2);
            Matrix yTest = getActualY(testPoints, randLine);
...
//            Utilities.printArr(Utilities.flattenArr(w));
        }
        numEpochs.add(j);
        return w;
    }
